    ...
    with st.sidebar:
        ...
        with col2:
            ...
            if permission and not st.session_state.user_location_accessed:
                st.info("🔄 Automatically fetching your location...")
                try:
                ...
                st.info("✅ Location already fetched. Use search to change.")
            else:
                st.info("🔒 Enable permission to automatically use your IP-based location (approximate).")
...
def display_advanced_features():
    ...
    st.markdown('<h3 class="futuristic-font neon-text">⚡ Advanced Tools</h3>', unsafe_allow_html=True)
   
    # Real-Feel Calculator in a tool card
    with st.expander("🌡️ Real-Feel Calculator", expanded=False):
        ...
        with col2:
            ...
            st.info("💡 Real-feel accounts for humidity and wind effects on perceived temperature.")
        st.markdown('</div>', unsafe_allow_html=True)
   
    # Pollen Details in expander
    with st.expander("🌿 Detailed Pollen Forecast", expanded=st.session_state.weather_data is not None):
        if st.session_state.weather_data:
            ...
            with col1:
                ...
                st.markdown(f'<h5>🌳 Tree Pollen</h5><p class="{level_class}"><strong>{pollen["tree"]}/10</strong> - {level_desc}</p>', unsafe_allow_html=True)
            ...
            with col2:
                ...
                st.markdown(f'<h5>🌱 Grass Pollen</h5><p class="{level_class}"><strong>{pollen["grass"]}/10</strong> - {level_desc}</p>', unsafe_allow_html=True)
            ...
            with col3:
                ...
                st.markdown(f'<h5>🌾 Weed Pollen</h5><p class="{level_class}"><strong>{pollen["weed"]}/10</strong> - {level_desc}</p>', unsafe_allow_html=True)
            ...
            st.markdown(f'<h4>📊 Overall Pollen Level</h4><p class="{overall_level_class}"><strong>{pollen["overall"]}/10</strong> - {overall_desc}</p>', unsafe_allow_html=True)
    ...
    with st.expander("⭐ Manage Favorites", expanded=False):
        st.markdown('<div class="tool-card">', unsafe_allow_html=True)
        if st.session_state.weather_data:
            if st.button("➕ Add Current Location", use_container_width=True):
